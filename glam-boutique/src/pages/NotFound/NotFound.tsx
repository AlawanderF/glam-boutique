import { Link } from 'react-router-dom';
import { motion } from 'framer-motion';
import { ArrowLeft } from 'lucide-react';
import { ROUTES } from '@/constants';

export default function NotFound() {
  return (
    <div className="container-app flex min-h-[70vh] flex-col items-center justify-center text-center">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
        <span className="font-display text-7xl text-ink-200">404</span>
        <h1 className="mt-4 font-display text-3xl text-ink-900">Página não encontrada</h1>
        <p className="mx-auto mt-3 max-w-md text-sm text-ink-500">
          O endereço que você tentou acessar não existe ou foi movido. Que tal voltar para a home e continuar
          explorando nossa coleção?
        </p>
        <Link
          to={ROUTES.home}
          className="mt-7 inline-flex items-center gap-2 text-sm font-semibold uppercase tracking-wide text-ink-900 hover:text-gold-700"
        >
          <ArrowLeft className="h-4 w-4" />
          Voltar para a home
        </Link>
      </motion.div>
    </div>
  );
}
