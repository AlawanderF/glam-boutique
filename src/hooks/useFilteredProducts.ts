...
export function useFilteredProducts(
  ...
  return useMemo(() => {
    ...
    if (filters.colors.length > 0) {
      // Normaliza para minúsculo sem acento para evitar falhas de comparação
      // (ex: 'Off-White' vs 'off-white', 'Azul-marinho' vs 'azul-marinho')
      const normalizedFilter = filters.colors.map((c) =>
        c.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
      );
      result = result.filter((p) =>
        p.colors.some((c) => {
          const normalizedColor = c.name
            .toLowerCase()
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '');
          return normalizedFilter.includes(normalizedColor);
        })
      );
    }
