...
import { productCountByCategoryId } from '@/constants/products';
...
export function CategoryCard({ category, index = 0, size = 'normal' }: CategoryCardProps) {
  // Usa a contagem real dos produtos carregados; cai no productCount da categoria
  // como fallback (útil quando a API retornar valores reais do banco).
  const realCount = productCountByCategoryId[category.id] ?? category.productCount;
  return (
    <motion.div
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-0 flex flex-col justify-end p-5 sm:p-6">
          <span className="eyebrow text-gold-300">{realCount} {realCount === 1 ? 'peça' : 'peças'}</span>
